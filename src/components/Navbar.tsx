...
export default function Navbar() {
    ...
    return (
        <div className="fixed w-full z-50 top-0 left-0">
            <div className="flex justify-between px-5 md:px-8 py-5">
                <div className="z-50" onClick={() => setIsOpen(!isOpen)}>
                    {!isOpen ? (
                        <button>
                            <FaBars size={30} className="fill-mono hover:fill-off-white" />
                        </button>
                    ) : (
                        <button>
            ...
            <div className={`transition-all ease-in-out duration-700 fixed inset-0 bg-black z-40 ${isOpen ? 'w-screen h-screen' : 'h-0'}`}>
                <div className={`flex flex-col justify-center gap-3 text-3xl font-bold items-center absolute w-full h-full px-8 transition-opacity duration-700 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
                    <div>
                        <a href="/" className="text-mono hover:text-off-white" onClick={handleClose}>HOME</a>
                    </div>
                    <div>
                        <a href="#about" className="text-mono hover:text-off-white" onClick={handleClose}>ABOUT</a>
                    </div>
                    <div>
                        <a href="#skill" className="text-mono hover:text-off-white" onClick={handleClose}>SKILLS</a>
                    </div>
                    <div>
                        <a href="#service" className="text-mono hover:text-off-white" onClick={handleClose}>SERVICES</a>
                    </div>
                    <div>
                        <a href="#project" className="text-mono hover:text-off-white" onClick={handleClose}>PROJECTS</a>
                    </div>
