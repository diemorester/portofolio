...
export default function Navbar() {
    ...
    const [isAfterProject, setIsAfterProject] = useState<boolean>(false);
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            const containerSection = document.getElementById("container-what-i-do");
            ...
            if (aboutSection && containerSection && projectSection) {
                ...
                if (window.scrollY >= projectSectionBottom) {
                    setIsAfterProject(false);
                    setIsScrolled(true);
                } else if (window.scrollY > containerSectionTop - 150) {
                    setIsAfterProject(true);
                    setIsScrolled(false);
                } else if (window.scrollY > aboutSectionBottom) {
                    setIsAfterProject(false);
                    setIsScrolled(true);
                } else {
                    setIsAfterProject(false);
        ...
        <div className="fixed w-full z-50 top-0 left-0">
            ...
            <div className={`transition-all ease-in-out duration-700 fixed inset-0 bg-black z-40 ${isOpen ? 'w-screen h-screen' : 'h-0'}`}>
                <div className={`flex flex-col justify-center gap-3 text-3xl font-bold items-center absolute w-full h-full px-8 transition-opacity duration-700 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
                    <div>
                        <a href="#hero-section" className="text-mono hover:text-off-white" onClick={handleClose}>HOME</a>
                    </div>
                    <div>
                        <a href="#about-section" className="text-mono hover:text-off-white" onClick={handleClose}>ABOUT</a>
                    </div>
                    <div>
                        <a href="#container-what-i-do" className="text-mono hover:text-off-white" onClick={handleClose}>SERVICES</a>
                    </div>
                    <div>
                        <a href="#stack-section" className="text-mono hover:text-off-white" onClick={handleClose}>STACKS</a>
                    </div>
                    <div>
                        <a href="#project-section" className="text-mono hover:text-off-white" onClick={handleClose}>PROJECTS</a>
                    </div>
                    <div className="flex gap-6 pt-6">
                        <a href="https://github.com/diemorester" target="_blank" rel="noopener noreferrer">
                            <IoLogoGithub size={35} className="fill-mono hover:fill-off-white" />
                        </a>
                        <a href="http://discordapp.com/users/388771161208782860" target="_blank" rel="noopener noreferrer">
                            <FaDiscord size={35} className="fill-mono hover:fill-off-white" />
                        </a>
                        <a href="https://wa.link/fw9mfb" target="_blank" rel="noopener noreferrer">
                            <FaWhatsapp size={35} className="fill-mono hover:fill-off-white" />
                        </a>
                    </div>
                </div>
            </div>
